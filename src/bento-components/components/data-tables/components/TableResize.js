...
function getParentOffsetLeft(tableEl) {
  ...
  let { offsetParent } = tableEl;
  while (offsetParent) {
    parentOffsetLeft += (offsetParent.offsetLeft || 0);
...
class TableResize extends React.Component {
  ...
  onResizeMove = (id, e) => {
    ...
    if (isResize) {
      ...
      const isFirstColumn = (selectableRows, id) => (selectableRows !== 'none' && id === 0) || (selectableRows === 'none' && id === 1);

      const isLastColumn = (id, finalCells) => id === finalCells.length - 2;
  ...
  render() {
    ...
    const {
      id, isResize, resizeCoords, tableWidth, tableHeight,
    } = this.state;
