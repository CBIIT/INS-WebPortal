...
import { widgetsData, facetSearchData } from './dashboardData';
...
export async function fetchAllFileIDsForSelectAll(fileCount = 100000) {
  ...
  const fetchResult = await client
    ...
    .then((result) => {
      const RESULT_DATA = getState().currentActiveTab === tabIndex[4].title ? 'patentOverView' : getState().currentActiveTab === tabIndex[3].title ? 'clinicalTrialOverView' : getState().currentActiveTab === tabIndex[2].title ? 'datasetOverView' : getState().currentActiveTab === tabIndex[1].title ? 'publicationsOverview' : 'projectsOverview';
      const fileIdsFromQuery = RESULT_DATA === 'projectsOverview' ? transformfileIdsToFiles(result.data[RESULT_DATA]) : RESULT_DATA === 'datasetOverView' ? transformCasesFileIdsToFiles(result.data[RESULT_DATA]) : result.data[RESULT_DATA] || [];
...
export async function singleCheckBox(payload) {
  ...
  const currentAllFilterVariables = allFilters;
  toggleCheckBoxWithAPIAction(payload, {
    ...currentAllFilterVariables,
    ..._.mergeWith({}, getState().bulkUpload, getState().autoCompleteSelection, customizer),
  });
...
const reducers = {
  ...
  UPDATE_CURRRENT_TAB_DATA: (state, item) => (
    {
      ...
      datatable: {
        ...state.datatable,
        dataProject: item.data.projectsOverview,
        dataPublication: item.data.publicationsOverview,
  ...
  //       filteredPatentIds: null,
  //       subjectOverView: {
  //         data: item.data.searchProjects.projectsOverview,
  //       },
  //       checkboxForAll: {
  //         data: checkboxData,
  //       },
